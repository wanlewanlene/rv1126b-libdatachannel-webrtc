...
def handle_offer(sdp_text):
    ...
    try:
        _, sdp = GstSdp.SDP.new_from_text(sdp_text)
...
def handle_ice(candidate, mid):
    if not candidate:
        return
    try:
        cand = GstWebRTC.WebRTCIceCandidate.new(mid, candidate)
        webrtc.emit('add-ice-candidate', cand)
    except Exception as e:
...
def on_ws_message(ws, message):
    ...
    if t == 'offer':
        ...
        GLib.idle_add(handle_ice, msg.get('candidate', ''), msg.get('mid'))
