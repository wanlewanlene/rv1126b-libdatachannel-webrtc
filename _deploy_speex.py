...
out = brun("cd /userdata/rtc && g++ -std=c++17 -O2 main.cpp -o bin/rv1126b_webrtc_push.new -I/userdata/rtc/ldc_install/include -L/userdata/rtc/lib -L/usr/local/lib -ldatachannel -lrockchip_mpp -lssl -lcrypto -lopus -lasound -lspeexdsp -pthread 2>&1; echo 'EXIT='$?", "编译 (Speex 降噪版)", 300)
if 'EXIT=0' not in out or 'error' in out.lower():
    brun("echo 编译失败", "失败", 10)
    board.close(); sys.exit(1)

# 2) 部署 + 重启 (信令/turn 保留)
brun("cd /userdata/rtc && cp -f bin/rv1126b_webrtc_push bin/rv1126b_webrtc_push.predenoise && cp -f bin/rv1126b_webrtc_push.new bin/rv1126b_webrtc_push && chmod +x bin/rv1126b_webrtc_push && echo 'DEPLOYED, old backed up as .predenoise'", "部署二进制", 20)
...
# 3) 验证
brun("grep -aE 'V4L2|encoder initialized|ALSA|AUDIO|denoise|streaming' /tmp/rtc_run.log | head -10", "启动日志", 30)
