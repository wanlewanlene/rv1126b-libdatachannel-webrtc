...
public:
    bool init(int width, int height, int fps, uint32_t bitrate) {
        width_ = width;
        height_ = height;
        hor_stride_ = (width + 15) & ~15;   // 16 字节对齐
        ...
        frame_size_ = hor_stride_ * ver_stride_ * 2;  // YUYV: 2 字节/像素
        ...
        std::cout << "[MPP] encoder initialized: H.264 " << width_ << "x" << height_
                  << " @" << fps << "fps " << (bitrate / 1024) << "kbps (input YUYV)" << std::endl;
        return true;
    }
    ...
    bool encode(const uint8_t* yuyv, size_t yuyv_size,
        ...
        uint8_t* dst = static_cast<uint8_t*>(mpp_buffer_get_ptr(enc_buf_));
        // 逐行拷贝 YUYV (每行 width*2 字节), 兼容 stride 对齐
        const uint8_t* src = yuyv;
        size_t row_bytes = width_ * 2;
        for (int i = 0; i < height_; ++i)
            memcpy(dst + i * hor_stride_ * 2, src + i * row_bytes, row_bytes);
...
public:
    bool start(const std::string& signaling_url, const std::string& room) {
        ...
        track_->setMediaHandler(std::make_shared<rtc::H264RtpPacketizer>(
            rtc::NalUnit::Separator::LongStartSequence, nullptr));
    ...
    void send(const uint8_t* data, size_t size) {
        if (!connected_ || !track_) return;
        track_->send(reinterpret_cast<const std::byte*>(data), size);
    }
...
int main() {
    ...
    while (g_running) {
        ...
        encoder.encode(frame_data, frame_size,
