...
brun("echo '== main.cpp 是否含 AGC disabled =='; grep -c 'AGC disabled 48k-bug' /userdata/rtc/main.cpp; echo '== 二进制 md5 =='; md5sum /userdata/rtc/bin/rv1126b_webrtc_push /userdata/rtc/bin/rv1126b_webrtc_push.new; echo '== 运行进程 =='; pgrep -af '[r]v1126b_webrtc_push' | grep -v bash || echo '(无进程)'; echo '== 日志 denoise 行 =='; grep -a 'denoise' /tmp/rtc_run.log", "核查部署", 20)
