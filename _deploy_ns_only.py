...
out = brun("cd /userdata/rtc && g++ -std=c++17 -O2 main.cpp -o bin/rv1126b_webrtc_push.new -I/userdata/rtc/ldc_install/include -L/userdata/rtc/lib -L/usr/local/lib -ldatachannel -lrockchip_mpp -lssl -lcrypto -lopus -lasound -lspeexdsp -pthread 2>&1; echo 'EXIT='$?", "编译 (禁用AGC)", 300)
...
# 3) 验证
brun("grep -aE 'ALSA|AUDIO|denoise|streaming' /tmp/rtc_run.log | head -6", "音频日志", 30)
